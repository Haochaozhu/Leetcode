...
public class StringToInteger {
    public int myAtoi(String str) {
        ...
        if (string.charAt(0) == '-' || string.charAt(0) == '+') {
            sign = string.charAt(0) == '-' ? -1 : 1;
            index += 1;
        } else if (string.charAt(0) - '0' > 9 || string.charAt(0) - '0' < 0) {
            return 0;
        }

        while (index < string.length()) {
            int temp = string.charAt(index) - '0';
            if (temp > 9 || temp < 0) return res * sign;
            if (res > Integer.MAX_VALUE / 10
                    || (res == Integer.MAX_VALUE / 10 && Integer.MAX_VALUE % 10 < temp)) {
