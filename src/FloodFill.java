...
public class FloodFill {
    ...
    private void fill(int[][] img, int i, int j, int newColor, int oriColor) {
        if (i < 0 || i >= img.length || j < 0 || j >= img[0].length) {
            return;
        }
        if (img[i][j] != oriColor) return;
        img[i][j] = newColor;
        fill(img, i + 1, j, newColor, oriColor);
        fill(img, i - 1, j, newColor, oriColor);
        fill(img, i, j + 1, newColor, oriColor);
        fill(img, i, j - 1, newColor, oriColor);
    }
}
