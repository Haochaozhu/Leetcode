 ...
 * called arithmetic if the sequence A[P0], A[P1], ..., A[Pk-1], A[Pk] is arithmetic.
    ...
    public int numberOfArithmeticSlices(int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                int d = (int)diff;
                int before = dp[j].getOrDefault(d, 0);
                int after = dp[i].getOrDefault(d, 0);
                count += before;
                dp[i].put(d, before + after + 1);
            }
        }
        return count;
    }
